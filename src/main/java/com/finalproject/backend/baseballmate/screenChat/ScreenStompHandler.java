...
@Component
public class ScreenStompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if(StompCommand.CONNECT == accessor.getCommand()) {
            ...
            String roomId = screenChatMessageService.getRoomId(Optional.ofNullable((String)message.getHeaders().get("simpDestination")).orElse("InvalidRoomId"));
            ...
            screenChatRoomService.setUserEnterInfo(sessionId, roomId, userId);
            screenChatMessageService.sendScreenChatMessage(ScreenChatMessage.builder()
                    .type(ScreenChatMessage.MessageType.ENTER)
                    .roomId(roomId)
                    .sender(user)
                    .build());
            ...
            String sessionId = (String) message.getHeaders().get("simpSessionId");
            String roomId = screenChatRoomService.getUserEnterRoomId(sessionId);
            // 저장했던 sessionId 로 유저 객체를 받아옴
            User user = screenChatRoomService.chkSessionUser(sessionId);
            String username = user.getUsername();
            screenChatMessageService. sendScreenChatMessage(ScreenChatMessage.builder()
                    .type(ScreenChatMessage.MessageType.QUIT)
                    .roomId(roomId)
                    .sender(user)
                    .build());
            // 퇴장한 클라이언트의 roomId 맵핑 정보를 삭제한다.
            screenChatRoomService.removeUserEnterInfo(sessionId);
            log.info("DISCONNECTED {}, {}", username, roomId);
            // 유저가 퇴장할 당시의 마지막 TALK 타입 메세지 id 를 저장함
            allScreenChatInfoService.updateReadMessage(user,roomId);
        }
        return message;
    }
}
